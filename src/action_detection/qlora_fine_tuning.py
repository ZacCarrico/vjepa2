...
"""
V-JEPA 2 QLoRA Fine-tuning for Action Detection

This script demonstrates QLoRA (Quantized Low-Rank Adaptation) fine-tuning for action detection
on the NTU RGB dataset. QLoRA combines 4-bit quantization of the base model with LoRA adapters,
enabling extremely memory-efficient fine-tuning while maintaining model quality.

Key differences from standard LoRA:
- Base model quantized to 4-bit precision (NF4 format)
- LoRA adapters trained in bfloat16
- ~4x memory reduction compared to full precision LoRA
...
from transformers import VJEPA2ForVideoClassification, VJEPA2VideoProcessor, BitsAndBytesConfig
...
class AdaptedLinear(nn.Module):
    ...
    During training, both original weights (frozen/quantized) and LoRA weights are used.
    """
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="V-JEPA 2 QLoRA Fine-tuning for Action Detection"
    )
...
def main():
    print("Starting V-JEPA 2 QLoRA Action Detection Fine-tuning")
    print("=" * 50)
    print("QLoRA: 4-bit Quantized Base Model + LoRA Adapters")
    print("=" * 50)
    ...
    if not torch.cuda.is_available():
        print("\n" + "=" * 50)
        print("ERROR: QLoRA requires CUDA")
        print("=" * 50)
        print("Current device: " + device)
        print("\nQLoRA uses bitsandbytes which only supports CUDA GPUs.")
        print("\nOptions:")
        print("  1. Run this script on a machine with CUDA GPU")
        print("  2. Use lora_fine_tuning.py for standard LoRA (no quantization)")
        print("=" * 50)
        raise RuntimeError(
            "QLoRA requires CUDA. Please use a CUDA-enabled GPU or run lora_fine_tuning.py instead."
        )
    ...
    # Create quantization config for 4-bit training
    ...
    if local_model_path.exists():
        ...
        # Save processor locally (model with quantization config can't be saved directly)
        processor.save_pretrained(local_model_path)
        print("Note: Quantized model weights are not cached locally")
    ...
    print("\nQuantized model parameter stats:")
    print_parameter_stats(model, "Quantized V-JEPA 2 (4-bit)")
    ...
    print_parameter_stats(model, "QLoRA Adapted V-JEPA 2")
    ...
    tensorboard_dir = f"runs/ntu_action_qlora_{output_suffix}"
    ...
    print(f"\nStarting QLoRA training at: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            # Save model to disk
            model_save_path = (
                models_dir / f"qlora_action_detection_{output_suffix}_epoch_{epoch}.pth"
            )
            torch.save(
                {
                    "epoch": epoch,
    ...
    metrics_filename = f"qlora_action_metrics_{output_suffix}.json"
    ...
    print_parameter_stats(model, "Final QLoRA Adapted Model")
    print(
        f"Memory efficiency: Used only {count_parameters(model)[1]:,} trainable parameters"
    )
    print(f"Base model quantized to 4-bit (NF4) for ~4x memory reduction")


def prepare_model_for_kbit_training(model):
    """
    Prepare model for k-bit training by enabling gradient checkpointing
    and ensuring compatibility between quantized weights and LoRA adapters.

    This is a simplified version - in practice, you'd use peft library's
    prepare_model_for_kbit_training() function.
    """
    # Enable gradient checkpointing (already done above, but kept for clarity)
    if hasattr(model, "gradient_checkpointing_enable"):
        model.gradient_checkpointing_enable()

    # For quantized models, we need to ensure input embeddings and output layers
    # require gradients (for classifier head training)
